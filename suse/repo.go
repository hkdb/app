...
func AddRepo(s, g string, restore bool) {
	...
	fmt.Println("\n" + name + " has been added...\n")
	// Record added repo
	if err := db.RecordRepo("zypper", name); err != nil {
...
func RemoveRepo(s string) {
	...
	exists, err := db.RepoExists("zypper", s)
	if err != nil {
		utils.PrintErrorExit("Error:", err)
	}
	if exists == false {
	...
	// Record added repo
	if err = db.RemoveRepo("zypper", sType, s); err != nil {
		utils.PrintErrorExit("Repo Remove Error:", err)
	}

	utils.DeleteDirIfEmpty(env.DBDir + "/packages/repo/local/zypper")
