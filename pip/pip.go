...
func AutoRemove() {
	...
	fmt.Println("This action does not apply to Pip...")
...
func ListSystemSearch(pkg string) {
	...
	listSearch := exec.Command(env.Env, env.Bash, "-c", mgr+" list |grep "+pkg)
...
func Upgrade() {
	...
	chkDep := exec.Command(env.Env, env.Bash, "-c", "pip-review")
