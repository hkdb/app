...
curl -o $HOME/.local/bin/app.zip https://github.com/hkdb/app/releases/download/$VER/app-linux-$CPUARCH.zip
if [[ $? -ne 0 ]] ; then
    echo -e "\n❌️ Failed to download app binary... Exiting...\n"
    exit 1
fi

echo -e "💫️ Installing binary...\n"
unzip -d $HOME/.local/bin/ $HOME/.local/bin/app-$USEROS-$CPUARCH.zip
mv $HOME/.local/bin/app-$USEROS-$CPUARCH $HOME/.local/bin/app

echo -e "**************"
echo -e " 💯️ ${GREEN}COMPLETED${NC}"
echo -e "**************\n"
